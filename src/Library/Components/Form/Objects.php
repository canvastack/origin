...
class Objects {
	...
	public function close($action_buttons = false, $option_buttons = false, $prefix = false, $suffix = false) {
		if ('show' !== $this->currentRouteName) {
			...
			$this->draw('<div class="canvastack-action-box">' . $prefix . $object . $suffix . '</div>');
		}
	}
	...
	private function setParams($function_name, $name, $value, $attributes, $label, $selected = false) {
		...
		if (!canvastack_string_contained(current_route(), 'show')) $attributes = self::checkValidationAttributes($name, $attributes);
	...
	private function alert_message($data = []) {
		...
		$session_messages = [];
		if (!is_empty(canvastack_sessions('get', 'message'))) $session_messages = canvastack_sessions('get', 'message');		
		$session_status   = null;
		if (!is_empty(canvastack_sessions('get', 'status')))  $session_status   = canvastack_sessions('get', 'status');
